...
function parseMessage(){
    $(".chatInfo li .talk_recordbox .talk_recordtext").each(function(){
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			var eTop = target.offset().top - 242;
			var eLeft = target.offset().left+1;
			var eWidth = target.width()+1;
		...
		function showEmotions(){
			var facenum = Math.round((target.width()+1-20)/42)*4;
			var category = arguments[0]?arguments[0]:'默认';
			var page = arguments[1]?arguments[1] - 1:0;
			$('#emotions .containerface').html('');
			$('#emotions .page').html('');
			cat_current = category;
			for(var i = page * facenum; i < (page + 1) * facenum && i < emotions[category].length; ++i){
			...
			for(var i = 1; i < emotions[category].length / facenum + 1; ++i){
