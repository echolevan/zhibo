...
class ViewController extends CommonController
{
    ...
    {
        $types = TypeArticle::get();
        return view('home.lecturer.view.create',compact('types'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->classify == 0){
            return back()->with('error','分类不能为空！')->withInput();
        }
        if(!preg_match("/^[a-zA-Z0-9]{8}$/",$request->description)){
            return back()->with('error','请填写规范的股票代码！')->withInput();
        }
